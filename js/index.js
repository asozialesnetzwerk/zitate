...
function getRandomZitatUrl() {
    ...
    } while (window.r[0][keys[z]] <= 0); //Bis positive Zitate gefunden
    
    var end = window.location.href.lastIndexOf('?');
    if(end < 0) end = window.location.href.length;
    return window.location.href.substring(0, end) + "?id=" + keys[z];
}
...
function checkLoad() {
    if (window.q.length == 2 && window.r.length == 1) {
        ...
        if(id.indexOf('-') < 1) window.location = getRandomZitatUrl();
        else {
            displayZitat(id);
            $(".get-quote").attr("href", getRandomZitatUrl()); //adds next zitat to button 
        }
    } 
}
